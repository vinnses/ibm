...
- Agent assignments: extractor / subagent / mechanical PDF field extraction / `gpt-5.6-luna` / medium / planned, activated only after this specification commit / five preserved documents with existing normalized ementas are a bounded extraction task. Primary / orchestrator, source reviewer and integrator / GPT-5 / effort unknown / actual / final source checks and integration remain in the user-supervised session.
