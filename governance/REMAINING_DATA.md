...
Updated 2026-09-05, baseline `11c549e`. **Planning only: every D-series batch below is not started. Wait for the user to choose the extent before execution.** No collection agent is active.
...
|---|---|---|---|---|
| D01 / first | Up to five already preserved Ficha 1 documents. Initial candidate codes: CI1001, CI1002, CI1003, CI1055, CI1215. Check existing W010 index and DInf transcription before extracting | New work-local `ementas.csv`: document ID/type, code, original title, literal ementa, source-stated hours/department/date if present, applicability status, path/hash/page and normalization notes. Copy existing verified transcription where available; extract only missing fields. Missing fields stay explicit | Luna/medium; Terra/medium only for a specific documentary ambiguity | Commit one document or at most five; no new Ficha search. Preserve indeterminate 2023 applicability. Further batches cover remaining preserved Ficha 1s, including BQ083/BF114/MN162, without treating chronology as continuity |
...
W017 only divides and publishes this backlog. All D01-D09 entries remain **not started / awaiting user selection**. Recommended first choice: D01, one batch of at most five preserved Ficha 1 documents, followed by D09 for the completed output only.
