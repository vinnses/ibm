...
<main class="grade">
  <div class="heading">
    <div><p class="eyebrow">Bacharelado em Informática Biomédica · UFPR</p><h1>Grade curricular <span>{data.year}</span></h1><p class="intro">Disciplinas por período, Fichas 1 localizadas e pistas de correspondência entre as duas grades.</p></div>
  ...
  <div class="overview" aria-label="Resumo da grade"><div><strong>8</strong><span>períodos</span></div><div><strong>{data.summary.coded}</strong><span>componentes com código</span></div><div><strong>{data.summary.fichas}</strong><span>com Ficha 1 localizada</span></div><div><strong>{data.summary.missing}</strong><span>sem Ficha 1 localizada</span></div></div>

  <div class="explain"><span class="state-dot located"></span><span>Ficha 1 localizada</span><span class="state-dot missing"></span><span>Não localizada na busca pública</span><span class="state-dot neutral"></span><span>Não se aplica</span></div>
            ...
            {#each items as component (component.id)}
              <article class="course" id={component.code}>
                ...
                <div class="course-bottom">
                  {#if component.fichaState === 'located'}
                    <span class="ficha-state"><span class="state-dot located"></span>Ficha 1 localizada</span>
                  {:else if component.fichaState === 'not_located'}
  ...
  <section class="sources"><div><h2>Documentos da grade</h2><p>Estes documentos definem a estrutura formal. As Fichas 1 são versões localizadas; sua aplicabilidade à grade indicada pode ainda exigir confirmação.</p></div><ul>{#each data.formalDocuments as document}<li><a href={document.url} target="_blank" rel="noopener">{document.title} ↗</a></li>{/each}</ul></section>
