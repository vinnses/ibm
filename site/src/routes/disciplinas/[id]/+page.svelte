...
<main>
  ...
  {#if data.occurrences.length}
    <ul>{#each data.occurrences as occurrence}<li><a href={`/conteudos/${occurrence.topic?.id}`}>{occurrence.topic?.label}</a> <StatusBadge value={occurrence.review_state} /> <StatusBadge value={occurrence.evidence_strength} /><p>{occurrence.evidence_text}</p><EvidenceList items={occurrence.evidence} /></li>{/each}</ul>
  {:else}<p class="meta">Nenhuma ocorrência analítica foi proposta no corpus utilizável do W033.</p>{/if}
  <h2>Dependências</h2>
