...
Checkpoint N3 of W028, updated by W030. This register reports the state of source families already present in the repository. A `structured` row means that a repository dataset or explicit register exists for the named factual scope; it does not mean that every field in every original source has been extracted. `partially_structured` means that only a bounded subset, transcription, index or source metadata is structured. `deferred_access` requires an authorized document or custodian. `deferred_tooling` is retained as historical vocabulary for the W028/W029 CPA tool restriction, superseded by W030's explicit Python authorization. `not_applicable` is used only where the family is outside the axis.
...
| CPA evaluation workbook | `structured` | [CPA page](../..//administracao/historico/fontes/paginas/cpa-avaliacao-cursos-2022-2026-09-04.html), [CPA workbook](../..//administracao/historico/fontes/documentos/cpa-avaliacao-curso-informatica-biomedica-2022.xlsx), [W030 extraction](../..//administracao/dados/cpa/README.md) | Use the faithful cell layer and source-linked normalized tables; do not recalculate formulas, combine response categories or infer a population frame. |
...
W030 supersedes the CPA `deferred_tooling` state under the user's explicit Python/openpyxl instruction. It also corrects W029's mistaken local-Ficha closure by representing twelve W022-W024 PDFs separately. Current generated totals are 217 indexed CSV datasets, 212 source records and 33 retained gap/search records. The final classification is in `governance/reviews/W030-gap-classification.csv`.
