...
Integrated snapshot after W019: 38 datasets, 191 source-manifest records and 33 human-question/search records. Counts are records, not unique documents or unresolved gaps. The historical-act batch is in `administracao/historico/atos-originais/`; two five-record Ficha 1 extractions are under `dados/curriculos/2023/fichas-1-lotes/`. Newer results coexist with earlier search records. Existing applicability and institutional-access limits are unchanged unless the original record and newer evidence explicitly establish otherwise.
