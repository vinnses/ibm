...
`permanent_fields` is a complete normalized transcription of the source table's hours, prerequisite/corequisite, modality, period nature, and applicable workload categories. `ementa`, `program`, `objectives`, `method`, `evaluation`, `bibliography`, and `teacher_fields` contain complete normalized transcriptions of all source-stated text in source order, including every bibliographic entry. `not stated` is used only where the source does not state a value; `indeterminado` is retained for curriculum applicability. `source_locators` identifies the PDF page and section for every field group. Normalization is limited to whitespace/line-break joining and removal of PDF rendering headers/footers; source spelling, punctuation, language, numbering, and order are retained. The external MN129 row retains the source's `2022.1` term, Nursing department, 45 total hours, complete bibliography, and responsible-teacher/signature-line text without treating it as a 2023 offering.
